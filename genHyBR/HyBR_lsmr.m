function [x_out, output] = HyBR_lsmr(A, b, P, options)
%
% [x_out, output] = HyBR_lsmr(A, b, P, options)
%
% HyBR_lsmr is a Hybrid Bidiagonalization Regularization method used for
...
%% Initialization
defaultopt = struct('InSolv','tikhonov','RegPar','wgcv','nLevel', 'est','Omega',...
  'adapt', 'Iter', [] , 'Reorth', 'off', 'x_true', 'off', 'BegReg', 2,...
  'FlatTol', 10^-10, 'MinTol', 4, 'ResTol', [10^-6, 10^-6]);
%   'FlatTol', 10^-6, 'MinTol', 4, 'ResTol', [10^-6, 10^-6]);
...
if nargin < 2
  error('HyBR_lsmr: Not Enough Inputs')
elseif nargin < 3
  P = {[],[]}; options = [];
elseif nargin < 4
  options = [];
...
for i = 1:maxiter+1 %Iteration (i=1) is just an initialization
  [U, B, V, alphakp1, vkp1] = feval(handle, A, U, B, V, P_le, P_ri, options, alphakp1, vkp1);
  ...
  if i >= 2 %Begin Lanczos iterations
    ...
    switch insolve
      ...
      case 'none'
        ...
        % Solve the projected problem with Tikhonov or TSVD
        %         [Ub, Sb, Vb] = svd(Bhat);
        %         [f, alpha] = feval('Tikhonovsolver', Ub, diag(Sb), Vb, vector, options);
        %         alpha
        %         %         f = B \ vector;
        alpha = 0;
      otherwise
        error('HyBR_lsmr error: No inner solver!')
    end
    x = V*f;
    % Test for a right preconditioner:
    if ~isempty(P_ri)
